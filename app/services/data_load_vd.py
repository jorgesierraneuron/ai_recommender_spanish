...
class books_load():
    def __init__(self,file_source, chunk_size, overlap, fuente, autor, open_ai_key): 
        ...
        self.sofia_db = qdrant_manager(embeding_config.QDRANT_URL.value,embeding_config.QDRANT_API_KEY.value)
    ...
    def saveDataset(self,documents,name):
        with open(f'datasets/{name}-clean.jsonl', 'w', encoding='utf-8') as f:
    ...
    def load_data(self,batch_size, data, collection_name):
        ...
        for i in range(0,len(data),batch_size):
            ...
            #create embeddings 
            embeds = self.embbedings(text_batch, 'openai')
            #payloads 
            payloads = [
                {
                    'text': x['text'],
